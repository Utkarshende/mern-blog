      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        {view === 'login' && (
          <div className="max-w-sm mx-auto bg-slate-900 border border-slate-800 p-10 rounded-3xl">
            <h2 className="text-2xl font-serif font-bold text-white mb-6">{authMode === 'login' ? 'Sign In' : 'Create Account'}</h2>
            <form onSubmit={async (e) => {
              ...
              try {
                ...
                if (authMode === 'login') {
                  ...
                  setView('home');
                } else { setAuthMode('login'); alert("Registered!"); }
              ...
              <p className="text-center text-[10px] cursor-pointer mt-4 uppercase text-slate-500 font-bold" onClick={() => setAuthMode(authMode === 'login' ? 'signup' : 'login')}>
                {authMode === 'login' ? 'No account? Join' : 'Have account? Login'}
              </p>
            </form>
          </div>
        )}
          ...
          <div className="max-w-2xl mx-auto">
            {publishedPosts.length === 0 ? (
              <div className="text-center py-24 border-2 border-dashed border-slate-800 rounded-3xl">
                <p className="text-slate-500 font-serif italic text-xl">The feed is currently empty...</p>
              </div>
            ) : (
              publishedPosts.map(post => (
                <article key={post._id} className="mb-24">
                  ...
                  <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-8 pb-4 border-b border-slate-800/50">
                    By {post.authorName} • {calculateReadingTime(post.content)} min read
                  </div>
        ...
        {view === 'drafts' && (
          <div className="max-w-2xl mx-auto">
            <h2 className="text-xl font-serif font-bold text-white mb-8">Your Private Drafts</h2>
            {draftPosts.map(post => (
              <div key={post._id} className="p-6 bg-slate-900 border border-slate-800 rounded-2xl mb-4 flex justify-between items-center">
                ...
                <button onClick={() => { setEditingId(post._id); setForm(post); setView('write'); }} className="text-blue-400 font-bold uppercase text-xs">Continue</button>
              </div>
            ))}
          </div>
        )}
